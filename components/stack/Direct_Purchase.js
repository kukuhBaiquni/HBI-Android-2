...
class DirectPurchase extends Component {
  constructor(props) {
    super(props)
  ...
  showSpecificModal() {
    ...
    this.setState({
      idProduct: navigation.state.params.id,
      id_Product: navigation.state.params._id,
      showModal: true,
      productName: navigation.state.params.productname,
      subtotal: navigation.state.params.subtotal,
      productPrice: this.props.cartTotal,
      productPhoto: navigation.state.params.photo,
      productProcess: navigation.state.params.process
    });
  }
                  ...
                  <View style={{flexDirection: 'row'}}>
                    ...
                    <View style={{height: 120, width: 140, marginTop: 10, paddingLeft: 10}}>
                      <Text style={{fontSize: 16, width: 140, textAlign: 'left', color: '#919191'}}>{this.state.productName}</Text>
                      {
                        ...
                        :
                        <Text style={{fontWeight: 'bold', marginTop: 5}}>{idrFormat(this.props.cartTotal)}</Text>
                      }
                      {/*Increment Button*/}
        ...
        <ScrollView style={{backgroundColor: '#d9d9d9'}}>
          <View style={styles.productWrapper}>
            <View style={styles.productHeader}>
              <Text style={styles.productName}>{navigation.state.params.productname}</Text>
            </View>
            <View style={styles.productDetails}>
              ...
              <View>
                <View style={{marginBottom: 3, flexDirection: 'row'}}>
                  ...
                  <Text style={{marginLeft: 10, color: '#9b9b9b'}}>{idrFormat(Number(this.props.cartTotal/this.state.qty))}</Text>
                </View>
                <View style={{marginBottom: 3, flexDirection: 'row'}}>
                ...
                <View style={{marginBottom: 3, flexDirection: 'row'}}>
                  ...
                  <Text style={{marginLeft: 10, color: '#9b9b9b'}}>{this.state.selected_process.includes('None') ? '-' : this.state.selected_process}</Text>
                </View>
                <View style={{marginBottom: 3, flexDirection: 'row'}}>
                  <Text style={{marginLeft: 10, fontWeight: 'bold'}}>Subtotal :</Text>
                  <Text style={{marginLeft: 10, color: '#9b9b9b'}}>{idrFormat(Number(this.props.cartTotal))}</Text>
                </View>
                <TouchableOpacity style={{marginTop: 10, flexDirection: 'row'}} onPress={() => this.showSpecificModal()}>
